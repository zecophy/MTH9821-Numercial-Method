...
// fixed depth binomial tree pricer for american put option with discrete dividend payment
...
std::vector<double> american_put_binomial_tree(Option& op, int N0, double tol = 1e-4)
{
	double old_value = american_put_fixed_depth_binomial_tree(op, N0)[0];
	int N = N0 * 2;
	double new_value = american_put_fixed_depth_binomial_tree(op, N)[0];
	std::vector<double> tmp;
	while (std::abs(new_value - old_value) > tol)
	{
		old_value = new_value;
		N *= 2;
		tmp = american_put_fixed_depth_binomial_tree(op, N);
		new_value = tmp[0];
	}
	tmp.push_back(N);
	return tmp;
