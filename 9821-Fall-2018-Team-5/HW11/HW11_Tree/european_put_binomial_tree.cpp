...
// fixed depth binomial tree pricer for european put option with discrete dividend payment
...
std::vector<double> european_put_binomial_tree(Option& op, int N0, double tol = 1e-4)
{
	double old_value = european_put_fixed_depth_binomial_tree(op, N0)[0];
	int N = N0 * 2;
	double new_value = european_put_fixed_depth_binomial_tree(op, N)[0];
	std::vector<double> tmp;
	while (std::abs(new_value - old_value) > tol)
	{
		old_value = new_value;
		N *= 2;
		tmp = european_put_fixed_depth_binomial_tree(op, N);
		new_value = tmp[0];
	}
	tmp.push_back(N);
	return tmp;
