...
// fixed depth binomial tree pricer for american call option with discrete dividend payment
std::vector<double> american_call_fixed_depth_binomial_tree(Option& op, int N)
{
	...
	double s_no_div_max = s_no_div0 * std::pow(u, N); //initialize for last period
	for (int i = 0; i <= N; i++)
	{
	...
	for (int k = N - 1; k >= 0; k--)
	{
		...
		double s_no_div_max = s_no_div0 * std::pow(u, k);
		if (present_period == num_div) // if it is last period, do not need to adjust price
		{
			// ordinary binomial tree for american put
			for (int i = 0; i <= k; i++)
			{
				s_div_vec[i] = s_no_div_max;
				V[i] = std::max(disc * (pu * V[i] + pd * V[i + 1]), std::max(-op.K + s_div_vec[i], 0.0));// American
				if (k <= 2)
				{
					option_result_vec[k][i] = V[i];
					stock_result_vec[k][i] = s_div_vec[i];
				}
				s_no_div_max = s_no_div_max / u / u;
			}
		}
		else// if it is not last period, adjust price backward
		{
			for (int i = 0; i <= k; i++)
			{
				s_div_vec[i] = s_no_div_max;
				...
				V[i] = std::max(disc * (pu * V[i] + pd * V[i + 1]), std::max(-op.K + s_div_vec[i], 0.0));
				if (k <= 2)
				{
					option_result_vec[k][i] = V[i];
					stock_result_vec[k][i] = s_div_vec[i];
	...
	double gamma = ((option_result_vec[2][0] - option_result_vec[2][1]) / (stock_result_vec[2][0] - stock_result_vec[2][1]) -
		(option_result_vec[2][1] - option_result_vec[2][2]) / (stock_result_vec[2][1] - stock_result_vec[2][2])) / ((stock_result_vec[2][0] - stock_result_vec[2][2]) / 2);
	double theta = (option_result_vec[2][1] - option_result_vec[0][0]) / (2 * dt);
	return { V[0], delta, gamma, theta };
}

std::vector<double> american_call_binomial_tree(Option& op, int N0, double tol = 1e-4)
{
	double old_value = american_call_fixed_depth_binomial_tree(op, N0)[0];
	int N = N0 * 2;
	double new_value = american_call_fixed_depth_binomial_tree(op, N)[0];
	std::vector<double> tmp;
	while (std::abs(new_value - old_value) > tol)
	{
		old_value = new_value;
		N *= 2;
		tmp = american_call_fixed_depth_binomial_tree(op, N);
		new_value = tmp[0];
	}
	tmp.push_back(N);
	return tmp;
